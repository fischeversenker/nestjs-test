...
Bootstraps your application by creating a new Nest.js app and telling it to listen for requests on port 3000.
...
Defines `class AppModule` that is decorated with Nest.js's `@Module()` decorator which receives a list of Controllers and Providers that are used throughout the module or provided to other consuming modules.
